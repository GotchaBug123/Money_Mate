...
const PRODUCT_INFO = {
    ...
    etf: {
        ...
        sections: [
            ...
            { heading: '🛒 ETF의 확실한 장점', body: '• 단돈 1~2만 원으로 대기업의 주주가 됩니다\n• 주식처럼 실시간으로 편리하게 사고팝니다\n• 계란이 알아서 나누어 담깁니다 (안전성)' },
...
const GUIDE_STEPS = [
    ...
    },
    {
        ...
        items: [
            { label: '리밸런싱 결과 확인', desc: '주가 변동에 따라 깨진 비중을 주기적으로 맞춰주는 리밸런싱 포트폴리오와 방치한 포트폴리오의 미래 자산 성장 곡선을 직접 비교하고, 목표 달성 확률을 확인할 수 있습니다.' },
        ],
    },
    {
        step: '4️⃣ STEP 4.',
...
const HeroIllustration = () => (
    <svg viewBox="0 0 420 260" xmlns="http://www.w3.org/2000/svg" className={styles.heroSvg}>
        ...
        <polyline points="198,210 225,195 252,200 279,178 306,165 333,150 360,140 375,130" fill="none" stroke="#1B5ED9" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        <polyline points="198,210 225,195 252,200 279,178 306,165 333,150 360,140 375,130 375,220 198,220" fill="#1B5ED9" stroke="none" opacity="0.07" />
    ...
    <div className={styles.modalOverlay} onClick={onClose}>
        <div className={styles.guideModalBox} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.guideModalBody}>
                <p className={styles.guideIntro}>저희는 유저님의 재무 상태를 진단해 최적의 분산투자 포트폴리오를 추천하고, 실제 자산을 등록해 황금 비율을 유지하도록 돕는 독립형 자산 관리 플랫폼입니다. 딱 4단계만 따라오시면 자산 관리의 감이 확실하게 잡힙니다!</p>
                {GUIDE_STEPS.map((s, i) => (
                ...
                <div className={styles.statsTop}>
                    <div>
                        <p className={styles.statsTitle}>
                            {chartTab === 'age'
                                ? '2026년 5월 기준, 다른 회원들은 얼마나 투자하고 있을까요? 💰'
                                : '나와 비슷한 성향의 투자자들은 얼마나 벌었을까요? 📈'}
                        </p>
                        <p className={styles.statsSub}>
                            {chartTab === 'age'
                                ? '전체 회원 42,318명의 실제 장바구니 데이터를 기반으로 집계된 통계입니다.'
                                : '전체 회원 42,318명의 투자 성향과 장바구니 데이터를 기반으로 분석한 결과입니다. (2026년 5월 기준)'}
                        </p>
                    </div>
                    <div className={styles.tabRow}>
                        <button className={`${styles.tabBtn} ${chartTab === 'age' ? styles.tabBtnOn : ''}`}
                                onClick={() => setChartTab('age')}>연령대별</button>
                        <button className={`${styles.tabBtn} ${chartTab === 'type' ? styles.tabBtnOn : ''}`}
                                onClick={() => setChartTab('type')}>투자성향별</button>
                    </div>
                </div>
                ...
                {chartTab === 'age' ? (
                    ...
                    <div className={styles.typeChartWrap}>
                            ...
                            <ResponsiveContainer width="100%" height={260}>
                                <BarChart layout="vertical" data={TYPE_DATA}
                                    ...
                                    <Bar dataKey="value" radius={[0, 4, 4, 0]} maxBarSize={22}
                                         label={{ position: 'right', fontSize: 11, fontWeight: 600, formatter: (v) => `${v >= 0 ? '+' : ''}${v}%`, fill: '#555' }}>
                                        {TYPE_DATA.map((entry, index) => (
                                            <Cell key={index} fill={entry.value >= 0 ? '#2563EB' : '#ef4444'} opacity={0.85} />
                                        ))}
                                    </Bar>
                                </BarChart>
                            </ResponsiveContainer>
                        ...
                        <div className={styles.insightCards}>
                            <div className={styles.insightCard} style={{ borderLeft: '3px solid #2563EB' }}>
                            ...
                            <div className={styles.insightCard} style={{ borderLeft: '3px solid #10b981' }}>
                            ...
                            <div className={styles.insightCard} style={{ borderLeft: '3px solid #f59e0b' }}>
