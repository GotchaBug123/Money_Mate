...
const StockLogo = ({ ticker, name, size=32 }) => {
    const [failed, setFailed] = useState(false);
    const domain = LOGO_MAP[ticker];
    const badge  = getBadge(ticker);
    if (!domain || failed) return (
        <div className={styles.logoBadge} style={{width:size,height:size,background:badge.bg,color:badge.color,fontSize:size*0.35,borderRadius:'50%'}}>
            {name.charAt(0)}
        </div>
    );
...
const MOCK_INDEX = [
    {label:'코스피',   value:'2,634.82',change:'+32.14',changeRate:'+1.24%',pos:true, chart:[0,4,6,10,12,16,18,22,24,20,22,26]},
    {label:'코스닥',   value:'728.15',  change:'-2.82', changeRate:'-0.38%',pos:false,chart:[24,22,20,18,16,14,12,10,12,8,6,4]},
    {label:'S&P 500',  value:'5,304.72',change:'+45.88',changeRate:'+0.87%',pos:true, chart:[4,6,4,10,8,14,12,18,16,20,22,26]},
    {label:'달러 환율',value:'1,342.50',change:'-1.60', changeRate:'-0.12%',pos:false,chart:[6,8,7,12,14,16,14,18,20,22,24,26]},
];

const MOCK_STOCKS = [
    {id:1, name:'삼성전자',        ticker:'005930',market:'국내',price:'78,400원',  chg:'+2.61%',pos:true, vol:'2.1조'},
    {id:2, name:'SK하이닉스',      ticker:'000660',market:'국내',price:'198,500원', chg:'-0.88%',pos:false,vol:'8,420억'},
    {id:3, name:'NVIDIA',          ticker:'NVDA',  market:'해외',price:'$1,208.88',chg:'+3.14%',pos:true, vol:'$18.4B'},
    {id:4, name:'TIGER 미국S&P500',ticker:'360750',market:'ETF', price:'18,245원', chg:'-0.42%',pos:false,vol:'3,820억'},
    ...
    {id:16,name:'POSCO홀딩스',     ticker:'005490',market:'국내',price:'392,000원', chg:'+0.33%',pos:true, vol:'4,200억'},
    {id:17,name:'에코프로비엠',    ticker:'247540',market:'국내',price:'118,500원', chg:'-2.11%',pos:false,vol:'6,800억'},
    {id:18,name:'KODEX 2차전지',   ticker:'305720',market:'ETF', price:'12,340원', chg:'-1.45%',pos:false,vol:'2,100억'},
    {id:19,name:'KODEX 반도체',    ticker:'091160',market:'ETF', price:'45,600원', chg:'+1.88%',pos:true, vol:'1,540억'},
    {id:20,name:'기아',            ticker:'000270',market:'국내',price:'98,400원',  chg:'-0.61%',pos:false,vol:'3,200억'},
];

const MOCK_THEMES = [
    {name:'AI 테마',     avg:'+2.1%',pos:true, stocks:[{n:'NVIDIA',c:'+3.14%',p:true},{n:'삼성전자',c:'+2.61%',p:true},{n:'SK하이닉스',c:'-0.88%',p:false}]},
    {name:'반도체 테마', avg:'+0.9%',pos:true, stocks:[{n:'SK하이닉스',c:'-0.88%',p:false},{n:'TSMC',c:'+1.22%',p:true},{n:'마이크론',c:'+0.74%',p:true}]},
    {name:'2차전지 테마',avg:'-1.2%',pos:false,stocks:[{n:'LG에너지솔루션',c:'-1.45%',p:false},{n:'POSCO홀딩스',c:'+0.33%',p:true},{n:'에코프로비엠',c:'-2.11%',p:false}]},
];
...
const Spark = ({ data, pos }) => {
    ...
    return (
        <svg width="100%" height={H} viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none">
            <polyline points={pts} fill="none" stroke={pos?'#0f6e56':'#a32d2d'} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
    );
};
                    ...
                    <div className={styles.ccTop}>
                        ...
                        <div className={styles.ccRight}>
                            <input className={styles.srch} placeholder="종목명 또는 티커 검색"
                                   value={query} onChange={e=>{setQuery(e.target.value);setShowCount(PAGE);}}/>
                        ...
                        <div className={styles.ftabs}>
                            {FILTERS.map(f=>(
                                <button key={f} className={`${styles.ftab} ${filter===f?styles.ftabOn:''}`}
                                        onClick={()=>{setFilter(f);setShowCount(PAGE);}}>{f}</button>
                            ))}
                        </div>
                        ...
                        <div key={s.id} className={styles.srow}>
                            ...
                            <div className={styles.sbts}>
                                <button className={`${styles.sbt} ${cart.includes(s.id)?styles.sbtCartOn:''}`}
                                        onClick={e=>toggleCart(e,s.id)} title="장바구니">🛒</button>
                                <button className={`${styles.sbt} ${likes.includes(s.id)?styles.sbtLikeOn:''}`}
                                        onClick={e=>toggleLike(e,s.id)} title="좋아요">❤️</button>
                            </div>
                        </div>
                    ))}
                ...
                <div className={styles.themeOverlay} onClick={()=>setThemePanel(null)}>
                  ...
                  {themePanel.pos?'↑':'↓'} {themePanel.avg} 평균
                        ...
                        <div className={styles.tpPeriods}>
                            {PERIODS.map(p=>(
                                <button key={p} className={`${styles.tpPeriod} ${detailPeriod===p?styles.tpPeriodOn:''}`}
                                        onClick={()=>setDetailPeriod(p)}>{p}</button>
                            ))}
                        </div>
                        <div className={styles.tpSortTabs}>
                            {DETAIL_SORTS.map(s=>(
                                <button key={s} className={`${styles.tpSortTab} ${detailSort===s?styles.tpSortTabOn:''}`}
                        ...
                        <div className={styles.tpStockList}>
                            {detailVisible.map(st=>(
                                <div key={st.id}
                                    ...
                                    <div className={`${styles.tpCheckbox} ${detailSelected.includes(st.id)?styles.tpCheckboxOn:''}`}>
                                        {detailSelected.includes(st.id)&&<span>✓</span>}
                                    </div>
                                    <StockLogo ticker={st.ticker} name={st.name} size={38}/>
            ...
            {divPanel && (
                <div className={styles.themeOverlay} onClick={()=>setDivPanel(null)}>
                    <div className={styles.themeDetailPanel} onClick={e=>e.stopPropagation()}>
                        ...
                        <div className={styles.tpSortTabs}>
                            {DIV_SORTS.map(s=>(
                                <button key={s} className={`${styles.tpSortTab} ${divSort===s?styles.tpSortTabOn:''}`}
