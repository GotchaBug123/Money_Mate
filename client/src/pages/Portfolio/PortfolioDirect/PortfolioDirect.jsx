...
const PortfolioDirect = () => {
    ...
    const validate = () => {
        if (!investAmount || !startDate || !endDate || !goalAmount) {
            alert('투자 금액, 기간, 목표 금액을 모두 입력해주세요.');
            return false;
        }
        if (!cart.length) {
            alert('최소 1개 이상의 종목을 담아주세요.');
            return false;
        }
        if (totalWeight !== 100) {
            alert(`비중 합계가 ${totalWeight}%입니다. 100%가 되어야 합니다.`);
            return false;
        }
        return true;
    };
                        ...
                        {cart.length === 0 ? (
                            ...
                            <div className={styles.cartList}>
                                    ...
                                    <div key={st.ticker} className={styles.cartItem}>
                                        ...
                                        <div className={styles.cartRight}>
                                            ...
                                            <button className={styles.removeBtn}
                                                    onClick={() => handleRemove(st.ticker)}>✕
                                            </button>
                                        </div>
                                    </div>
                                ))}
                                ...
                                <div className={styles.weightFooter}>
                                    <span
                                        className={`${styles.weightTotal} ${totalWeight !== 100 ? styles.weightErr : styles.weightOk}`}>
                                ...
                                return (
                                    <div key={st.ticker} className={styles.resultItem}>
                                        ...
                                        <div className={styles.resultRight}>
                                            <span
                                                className={st.pos ? styles.resultPos : styles.resultNeg}>{st.chg}</span>
            ...
            {showNameModal && (
                <div className={styles.overlay} onClick={() => setShowNameModal(false)}>
                    <div className={styles.nameModal} onClick={e => e.stopPropagation()}>
                        ...
                        <input className={styles.nameInput} type="text" placeholder="예: 내 첫 성장주 포트폴리오"
                               value={portfolioName} onChange={e => setPortfolioName(e.target.value)}
                               onKeyDown={e => e.key === 'Enter' && handleModalConfirm()} autoFocus/>
                        <div className={styles.modalBtns}>
                            <button className={styles.modalCancelBtn} onClick={() => setShowNameModal(false)}>취소
                            </button>
