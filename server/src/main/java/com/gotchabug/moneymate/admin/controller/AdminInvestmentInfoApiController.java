...
@Tag(name = "관리자 투자정보관리 API", description = "관리자 페이지 > 투자정보관리 탭 데이터 API")
public class AdminInvestmentInfoApiController {
    ...
    private boolean isNotAdmin(HttpSession session) {
        ...
        if (!(loginUser instanceof Member member)) return true;
        return !"ADMIN".equalsIgnoreCase(member.getRole());
    }


    /* ──────────────────────────────────────────────
       1. 상단 요약 통계 (관리 회원 수 / 보유 종목 수 / 총 평가금액)
       ────────────────────────────────────────────── */
    @Operation(
    ...
    @GetMapping("/summary")
    public ResponseEntity<AdminInvestmentSummaryResponse> getSummary(HttpSession session) {
        if (isNotAdmin(session)) {
            return ResponseEntity.status(403).build();
        }
        return ResponseEntity.ok(adminInvestmentInfoService.getSummary());
    }


    /* ──────────────────────────────────────────────
       2. 회원별 투자 현황 목록 (회원투자정보 탭)
       ────────────────────────────────────────────── */
    @Operation(
    ...
    public ResponseEntity<List<AdminMemberInvestmentResponse>> getMemberList(
            ...
            @Parameter(description = "투자 성향 코드 필터 (예: AGGRESSIVE / MODERATE / CONSERVATIVE 등). 전체 조회 시 생략")
        ...
        return ResponseEntity.ok(
                adminInvestmentInfoService.getMemberInvestmentList(keyword, riskTypeCode)
        );
    }


    /* ──────────────────────────────────────────────
       3. 연령대별 평균 투자 성향 통계 (통계 탭)
       ────────────────────────────────────────────── */
    @Operation(
    ...
    public ResponseEntity<List<AdminAgeGroupStatResponse>> getAgeGroupStats(HttpSession session) {
        if (isNotAdmin(session)) {
            return ResponseEntity.status(403).build();
        }
        return ResponseEntity.ok(adminInvestmentInfoService.getAgeGroupStats());
    }
}
